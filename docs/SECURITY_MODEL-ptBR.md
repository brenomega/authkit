...
Todo JWT aceito contém `token_use` explícito. `first_party_access`, `oauth_access` e `id_token` têm audiences e consumidores distintos. APIs first-party exigem `jti` de sessão viva e `session_version` numérico aceito pela fronteira PostgreSQL sem cache; OAuth/ID token nunca é aceito nessas rotas. Endpoints de alto risco possuem throttles específicos; produção exige origins HTTPS e peers proxy/CDN explícitos, sem confiar em headers enviados fora desses peers.
