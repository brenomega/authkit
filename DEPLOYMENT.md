...
Required only when `AUTH_EMAIL_OUTBOX_DISPATCH_MODE=queue`, which is the default, or when direct mode explicitly preserves Rabbit observability for unrelated host-app Rabbit usage.
...
* `AUTH_EMAIL_OUTBOX_LOCK_TTL_SECONDS`: Time before an abandoned `PROCESSING` email is eligible for retry. In direct mode AuthKit uses an effective lock no lower than the configured value and large enough to cover accepted local executor backlog. Default: `300`.
* `AUTH_EMAIL_OUTBOX_DELIVERY_ACK_TIMEOUT_SECONDS`: Time a `QUEUED` external delivery may wait for provider acceptance before becoming claimable again. In queue mode this covers Rabbit listener acknowledgment; in direct mode this starts when a direct worker is about to call the provider. Default: `600`.
...
* `AUTH_EMAIL_OUTBOX_PRESERVE_RABBIT_OBSERVABILITY`: Keeps Spring Boot Rabbit health and metrics auto-configuration active in direct mode for host-app Rabbit usage. Default: `false`.
...
* **Email Delivery:** Registration, recovery, and password-change emails are first written into the transactional `email_outbox` table. In `queue` mode, the dedicated email scheduler publishes due rows to RabbitMQ after commit and marks them `QUEUED`; the Rabbit listener marks them `SENT` only after the configured provider returns acceptance and a provider message id. Queue publish failures, provider failures, and delivery-ack timeouts are retried with backoff, and poison messages can dead-letter to `authkit.email.dlq`, so RabbitMQ latency does not hold user database transactions open. In `direct` mode, the scheduler claims only `AUTH_EMAIL_OUTBOX_DIRECT_BATCH_SIZE` rows and submits provider calls to the bounded direct dispatch worker pool while rows remain protected by an effective `PROCESSING` lock; the worker marks `QUEUED` immediately before provider I/O, then marks `SENT` on provider acceptance or `FAILED` on provider failure. AuthKit email Rabbit listener/publisher beans are not loaded in this mode, and Rabbit health/metrics auto-configuration is suppressed by default so an unused broker cannot affect readiness. Spring Boot Rabbit connection/template auto-configuration remains available for unrelated Rabbit usage; set `AUTH_EMAIL_OUTBOX_PRESERVE_RABBIT_OBSERVABILITY=true` only when that unrelated usage should also contribute Rabbit health and metrics.
